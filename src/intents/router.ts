...
const TRAINING_PLAN_QUESTION = /(?:訓練|菜單|課表|跑量|配速|間歇|恢復|週期)[^？?]*(?:怎麼|如何|安排|規劃|多少)/i;
...
export function classifyIntent(text: string): RoutedIntent {
  return WEATHER_KEYWORDS.test(text) ||
    (WEATHER_TIME_SIGNALS.test(text) && RUNNING_SUITABILITY.test(text) && !TRAINING_PLAN_QUESTION.test(text))
    ? "weather"
    : "general";
}
