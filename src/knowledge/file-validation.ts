...
async function validateDocxZip(bytes: Uint8Array): Promise<void> {
  ...
  for(let i=0;i<count;i++){
    ...
    if(local+30>centralOffset||view.getUint32(local,true)!==0x04034b50||view.getUint16(local+6,true)!==flags||view.getUint16(local+8,true)!==method)throw new KnowledgeFileError("invalid_file");const lnl=view.getUint16(local+26,true),lel=view.getUint16(local+28,true);if(decode(bytes.slice(local+30,local+30+lnl))!==name)throw new KnowledgeFileError("invalid_file");const start=local+30+lnl+lel;if(size>MAX_BYTES||start+compressed>centralOffset)throw new KnowledgeFileError("invalid_file");
    if(required.has(name)){if(!size)throw new KnowledgeFileError("invalid_file");const raw=bytes.slice(start,start+compressed);const data=method===0?raw:await inflate(raw,"deflate-raw",size);if(data.length!==size||crc32(data)!==view.getUint32(offset+16,true))throw new KnowledgeFileError("invalid_file");const xml=decode(data).replace(/^\uFEFF?\s*(?:<\?[\s\S]*?\?>\s*)?(?:(?:<!--[\s\S]*?-->)\s*)*/,"");const local=required.get(name)!;if(!new RegExp(`^<(?:[A-Za-z_][\\w.-]*:)?${local}(?=[\\s/>])`).test(xml))throw new KnowledgeFileError("invalid_file");required.delete(name);}
    offset+=46+nl+el+cl;
  }
