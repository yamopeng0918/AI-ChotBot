...
function setup(options: { queueFails?: boolean; duplicate?: boolean } = {}) {
  const order: string[] = [];
  const repository = {
    listDocuments: vi.fn(), getDocument: vi.fn(), uploadExists: vi.fn().mockResolvedValue(Boolean(options.duplicate)),
    createPendingDocumentWithJob: vi.fn(async () => { order.push("d1"); return { created: true }; }),
...
describe("POST /admin/knowledge/files", () => {
  test("validates, stores, atomically creates metadata, then enqueues IDs only", async () => {
    ...
    expect(d.order).toEqual(["validate", "r2", "d1", "queue"]);
  ...
  test("replays duplicate request without side effects", async () => {
    ...
    expect(d.validateFile).not.toHaveBeenCalled(); expect(d.objectStore.putOriginal).not.toHaveBeenCalled();
    expect(d.repository.createPendingDocumentWithJob).not.toHaveBeenCalled(); expect(d.ingestionQueue.send).not.toHaveBeenCalled();
  });
...
test("atomically creates a document/job and recognizes the stable duplicate in real D1", async () => {
  ...
  try {
    ...
    await expect(repository.createPendingDocumentWithJob(document, job)).resolves.toEqual({ created: true });
    await expect(repository.createPendingDocumentWithJob(document, job)).resolves.toEqual({ created: false });
