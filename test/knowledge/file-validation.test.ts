...
const validDocx = zipEntries([["[Content_Types].xml", "<Types></Types>"], ["word/document.xml", "<w:document></w:document>"]]);
...
function validPng() {
  const sig=new Uint8Array([137,80,78,71,13,10,26,10]); return join(sig,pngChunk("IHDR",new Uint8Array([0,0,0,1,0,0,0,1,8,2,0,0,0])),pngChunk("IDAT",new Uint8Array([1])),pngChunk("IEND",new Uint8Array()));
}
